---
title: WDA Signing with Xcode for iOS and tvOS
hide:
  - navigation
---

The Appium device farm now allows you to manage real iOS and tvOS devices on non-Mac systems, including Raspberry Pi. To optimize its functionality, you need to re-sign the WebDriver agent with your provisioning profile and upload it to the device farm. The device farm will handle all the internal configuration.

There are two approaches to set up WebDriverAgent for iOS and tvOS devices:

1. **Device Farm Managed WDA Build** - The device farm can automatically build, re-sign, and upload the WDA for you using a Mac node machine.
2. **Manual WDA Setup** - You can manually build, re-sign, and upload the WDA using your own Mac machine.

Choose the approach that best fits your setup. Below are detailed instructions for both methods.

## Device Farm Managed WDA Build
...
### Building WebDriverAgent from source
